class Emitter{
  ...
  void renderReflection(){
    ...
    if( yPer > .05 )
      renderImageOnFloor( new Vec3D( pos.x, floorLevel, pos.z ), radius * 10.0, color( 0.5, 1.0, yPer*.25 ), yPer );

    if( mousePressed )
      renderImageOnFloor( new Vec3D( pos.x, floorLevel, pos.z ), radius + ( yPer + 1.0 ) * radius * random( 2.0, 3.5 ), color( 1.0, 0, 0 ), yPer );
  }
