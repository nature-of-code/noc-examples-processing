...
class Particle {
  ...
  Vec3D startPos;     // just used to make sure every loc[] is initialized to the same position
  ...
  Particle( int _gen, Vec3D _pos, Vec3D _vel ) {
    gen         = _gen;
    radius      = random( 10 - gen, 50 - ( gen-1)*10 );

    len         = (int)( radius*.5 );
    pos         = new Vec3D[ len ];
    startPos    = new Vec3D( _pos.add( new Vec3D().randomVector().scaleSelf( random( 1.0 ) ) ) ); 
  ...
  void findPerlin() {
    float xyRads      = getRads( pos[0].x, pos[0].z, 20.0, 50.0 );
    float yRads       = getRads( pos[0].x, pos[0].y, 20.0, 50.0 );
  ...
  void render() {
    ...
    renderImage(images.particle, pos[0], radius * agePer, c, 1.0 );

    // Rendering two graphics here. Makes the particles more vivid,
    // but will hinder the performance.
    c = color( 1, agePer, agePer );
    renderImage(images.particle, pos[0], radius * agePer * .5, c, agePer );
  }

  void renderReflection() {
    float altitude           = floorLevel - pos[0].y;
    float reflectMaxAltitude = 25.0;
    float yPer               = ( 1.0 - ( altitude/reflectMaxAltitude ) ) * .5;

    if ( yPer > .05 )
      renderImageOnFloor(images.particle, new Vec3D( pos[0].x, floorLevel, pos[0].z ), radius * agePer * 8.0 * yPer, color( agePer, agePer*.25, 0 ), yPer + random( .2 ) );
  }
